-- Use IST calendar dates for the RRR queue, order history, and AI carry-over.
CREATE OR REPLACE VIEW public.v_rrr_queue WITH (security_invoker = true) AS
 SELECT c.id AS customer_id,
    ...
    c.lifetime_value,
        CASE
            WHEN c.lifetime_orders > 0 THEN round(c.lifetime_value / c.lifetime_orders::numeric, 2)
            ELSE NULL::numeric
        END AS aov,
   ...
   FROM customers c
     ...
     LEFT JOIN LATERAL ( SELECT count(*) AS attempts,
            max(f.completed_at AT TIME ZONE 'Asia/Kolkata')::date AS last_contacted_on,
            (array_agg(f.outcome ORDER BY f.completed_at DESC NULLS LAST))[1] AS last_outcome,
            (min(f.due_at AT TIME ZONE 'Asia/Kolkata') FILTER (WHERE f.completed_at IS NULL))::date AS next_due_on,
            (array_agg(f.id ORDER BY f.due_at) FILTER (WHERE f.completed_at IS NULL))[1] AS open_followup_id
           FROM followups f
          WHERE f.customer_id = c.id) fu ON true
     ...
     LEFT JOIN LATERAL ( SELECT
                CASE
                    WHEN bool_and(o.payment_state = 'paid'::payment_state) THEN 'Prepaid only'::text
                    WHEN bool_and(o.payment_state = 'unpaid'::payment_state) THEN 'COD only'::text
                    ELSE 'Mixed'::text
                END AS profile
           FROM orders o
          WHERE o.customer_id = c.id) pay ON true
  WHERE c.merged_into_id IS NULL AND c.lifetime_orders > 0;;

CREATE OR REPLACE VIEW public.v_rrr_customer_orders WITH (security_invoker = true) AS
 SELECT o.customer_id,
   ...
   FROM orders o
     ...
     LEFT JOIN LATERAL ( SELECT string_agg(((p.name || COALESCE(' '::text || p.variant, ''::text)) || '×'::text) || oi.quantity, ', '::text ORDER BY p.sort_order) AS products
           FROM order_items oi
