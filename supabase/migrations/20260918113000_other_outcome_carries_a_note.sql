-- "Other" — the call that does not fit any of the eight buttons.
--
-- The floor kept hitting the same wall: a customer says something the dialog
-- has no word for (shifted city, in hospital, family function, wants a
-- different product, call me after the festival). The rep had to pick the
-- least wrong button, which put a lie in the outcome column and the real
-- sentence in the note. So the outcome becomes 'other' and the note carries
-- what happened.
--
-- Two rules make it safe to offer:
--   * the note is required — an "other" with nothing written is an outcome
--     nobody can read afterwards, which is the very thing this fixes;
--   * the next follow-up is tomorrow, fixed, exactly like "interested". What
--     the customer said has to be picked up the next morning, so the lead
--     comes back in Action due instead of living in one rep's memory.
--
-- Down: supabase/migrations/20260918113000_other_outcome_carries_a_note.down.sql
...
alter table followups add constraint followups_outcome_check check (
  outcome is null or outcome in (
    ...
    'order_placed','medicine_not_finished','will_update_later','other'
  )
);

comment on column followups.outcome is
  'Controlled call outcome. order_placed is the conversion signal — it means an order was actually placed on this call, not that one was promised (will_buy). other means the call fitted no button: its remark is required and says what happened.';

-- The generic validator behind every assigned RRR call. Only its accepted-value
-- list changes here; the body is otherwise the one 20260914120127 created.
...
begin
  ...
  if p_outcome not in ('order_placed','will_buy','medicine_not_finished',
      'will_update_later','no_answer','not_interested','wrong_number','connected',
      'busy','other') then
