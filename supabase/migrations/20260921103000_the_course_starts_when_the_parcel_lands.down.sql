-- Roll back to the order-date clock: the view loses its derived dates and the
-- generator goes back to counting the refill window from the day the order was
-- placed, with no medicine gate on the other buckets.
--
-- create or replace cannot drop a view's trailing columns, so the view is
-- dropped and rebuilt. Nothing depends on it but the RRR screens, which read
-- it by name at request time.
